...
template <typename T> void nMinMaxOf(unsigned long long& outMin, unsigned long long& outMax, const unsigned long long total, const T values[]) {
	outMin = outMax = 0;
...
public:
	...
	[[nodiscard]] String toString() const noexcept { return L"Vector3D: (" + std::to_wstring(x) + L", " + std::to_wstring(y) + L", " + std::to_wstring(z) + L")"; }
};
...
public:
	...
	[[nodiscard]] String toString(const bool simplify = false) const noexcept { return simplify ? L"Vector2D: (" + std::to_wstring(x) + L", " + std::to_wstring(y) + L")" : L"Vector2D: (" + std::to_wstring(x) + L", " + std::to_wstring(y) + L")"; }
};
...
public:
	...
	Matrix4D& transpose() noexcept {
		...
		temp = a13, a13 = a31, a31 = temp;
		temp = a14, a14 = a41, a41 = temp;
		temp = a23, a23 = a32, a32 = temp;
		temp = a24, a24 = a42, a42 = temp;
		temp = a34, a34 = a43, a43 = temp;
		return *this;
	}
...
inline void DirectMatrixFromMatrix(const Matrix4D& in, DirectX::XMFLOAT4X4& out) noexcept {
	out._11 = static_cast<float>(in.a11);
	out._12 = static_cast<float>(in.a12);
	out._13 = static_cast<float>(in.a13);
	out._14 = static_cast<float>(in.a14);
	out._21 = static_cast<float>(in.a21);
	out._22 = static_cast<float>(in.a22);
	out._23 = static_cast<float>(in.a23);
	out._24 = static_cast<float>(in.a24);
	out._31 = static_cast<float>(in.a31);
	out._32 = static_cast<float>(in.a32);
	out._33 = static_cast<float>(in.a33);
	out._34 = static_cast<float>(in.a34);
	out._41 = static_cast<float>(in.a41);
	out._42 = static_cast<float>(in.a42);
	out._43 = static_cast<float>(in.a43);
	out._44 = static_cast<float>(in.a44);
}
#endif


template <unsigned long long N> constexpr bool is2Power = N != 0 && (N & N - 1) == 0;

template <unsigned int totalCount> requires is2Power<totalCount> || (totalCount == 0)
class Average;

template <> class Average<0> {
protected:
	...
	virtual void push(double) noexcept = 0;
	virtual void clear() noexcept = 0;
	[[nodiscard]] virtual double get() noexcept = 0;
	[[nodiscard]] virtual bool isFull() const noexcept = 0;
};

template <unsigned int totalCount> requires is2Power<totalCount>
class Average<totalCount> final : public Average<0> {
	double values[totalCount] {};
	double sum = 0;
...
public:
	...
	void push(double value) noexcept override {
		if (isFull()) sum -= values[nextIndex];
		else ++currentCount;
	...
	[[nodiscard]] double get() noexcept override {
		if (currentCount) return sum / currentCount;
		return 0.0;
	}

	[[nodiscard]] bool isFull() const noexcept override { return currentCount == totalCount; }
};
